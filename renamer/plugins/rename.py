...
async def media(c, m):
    ...
    file_name = await c.ask(chat_id=m.from_user.id, text="Send me the New FileName for this file or send /cancel to stop   IF YOUR FILE'S NAME IS EXAMPLE.MP4   THE NEW NAME SHOULD LOOK LIKE  ANY_NAME.MP4  \n   NEVER CHANGE FILE EXTENSIONS(MP4 , MKV ,WEBM , etc.", filters=filters.text)
